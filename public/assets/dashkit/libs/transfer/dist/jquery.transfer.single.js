...
var Transfer = (function ($) {
    ...
    function transfer(settings) {
        ...
        var itemText = '项';
        //选项文本
        var itemLabelText = settings.itemLabelText;
        if (itemLabelText == "") {
            itemLabelText = '项';
        }

        //选中的文本
        var itemSelectedLabelText = settings.itemSelectedLabelText;
        if (itemSelectedLabelText == "") {
            itemSelectedLabelText = '选中';
        }

        //Groups label
        var itemGroupLabelText = settings.itemGroupLabelText;
        if (itemGroupLabelText == "") {
            itemGroupLabelText = '选中';
        }
        ...
        console.log(itemLabelText);
        // 穿梭框渲染
        $(container).append(generateTransfer(inputId, currentTimeStr, settings));
        ...
        $(transferId).find(tabItemName).on("click", function () {
            ...
            if (!$(this).is(".tab-active")) {
                ...
                // 清空右侧数量
                $(transferId).find(selectedTotalNum).text("共 0 个");
                // 未选中项
                if ($(transferId).find(tabContentFirst).css("display") != "none") {
                    ...
                    $(transferId).find(groupTotalNum).empty();
                    $(transferId).find(groupTotalNum).append($(transferId).find(transferDoubleGroupListLiUlLi).length + itemText);
                } else {
                    // 分组
                    ...
                    $(transferId).find(totalNum).empty();
                    $(transferId).find(totalNum).append($(transferId).find(transferDoubleListLi).length + itemText);
                }
                // 数据变化触发回调
        ...
        $(addSelected).on("click", function () {
            ...
            if ($(transferId).find(tabContentFirst).css("display") != "none") {
                ...
                // 右侧数量
                $(transferId).find(selectedTotalNum).text(selected_total_num + itemText);
     ...
     * @returns {string}
     */
    function generateTransfer(inputId, currentTimeStr, settings) {
        var htmlStr =
            ...
            '<div class="tab-item-name tab-item-name-' + currentTimeStr + ' tab-active">' + settings.itemGroupLabelText + '</div>' +
            '<div class="tab-item-name tab-item-name-' + currentTimeStr + '">' + settings.itemLabelText + '</div>' +
            '</div>' +
            ...
            '<div class="param-item">' + settings.itemSelectedLabelText + '</div>' +
            '</div>' +
